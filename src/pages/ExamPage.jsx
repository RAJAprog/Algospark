...
//   - Proctoring (fullscreen + tab visibility) covers both phases.
//
// FIX — Duration always fell back to 60 min:
//   ROOT CAUSE: ExamEditor saved the field as "durationInMinutes" but ExamPage
//   read "durationMinutes", so exam?.durationMinutes was always undefined → ?? 60.
//   FIX: ExamEditor now saves the key as "durationMinutes".
//        ExamPage reads: exam?.durationMinutes ?? exam?.durationInMinutes ?? 60
//        so both new and old Firestore docs work correctly.
...
// ── FIX helper: safely read duration from either field name ──────────────────
// ExamEditor previously saved "durationInMinutes"; it now saves "durationMinutes".
// We support both so old exam docs in Firestore still work.
function getExamDurationMinutes(exam) {
  return exam?.durationMinutes ?? exam?.durationInMinutes ?? 60;
}
...
export default function ExamPage() {
  ...
  const [confirmMCQSubmit, setConfirmMCQSubmit]  = useState(false);
  ...
  const totalDuration = getExamDurationMinutes(exam) * 60;
  ...
  if (phase === 'coding') {
    ...
    // ── FIX: use helper for the duration display in finish modal ──
    const examDurationMins = getExamDurationMinutes(exam);
          ...
          <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.85)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1000 }}>
            <div style={{ background:'#161b22', border:'1px solid #30363d', borderRadius:16, padding:36, maxWidth:460, width:'90%', textAlign:'center' }}>
              ...
              <div style={{ background:'#0d1117', borderRadius:10, padding:'12px 16px', marginBottom:24, textAlign:'left' }}>
                ...
                {/* ── FIX: show actual duration, not hardcoded 60 ── */}
                <p style={{ fontSize:13, color:'#e6edf3', marginTop:4 }}>
                  Duration: <strong style={{ color:'#58a6ff' }}>{examDurationMins} min</strong>
                </p>
                {evaluatedCount < codingQuestions.length && (
                  <p style={{ fontSize:12, color:'#f0883e', marginTop:6 }}>
                    ⚠ {codingQuestions.length - evaluatedCount} coding question(s) not yet submitted. Unevaluated questions score 0.
