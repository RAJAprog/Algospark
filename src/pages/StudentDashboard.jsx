...
//   daily:       <Icon d={<><rect x="1.5" y="3" width="13" height="11.5" rx="1.5"/><line x1="1.5" y1="6.5" x2="14.5" y2="6.5"/><line x1="5" y1="1.5" x2="5" y2="4.5"/><line x1="11" y1="1.5" x2="11" y2="4.5"/><polyline points="5 10 7 12 11 8.5"/></>} />,
//   weekly:      <Icon d={<><rect x="1.5" y="3" width="13" height="11.5" rx="1.5"/><line x1="1.5" y1="6.5" x2="14.5" y2="6.5"/><line x1="5" y1="1.5" x2="5" y2="4.5"/><line x1="11" y1="1.5" x2="11" y2="4.5"/><line x1="4" y1="9.5" x2="12" y2="9.5"/><line x1="4" y1="11.5" x2="10" y2="11.5"/></>} />,
...
//   { id:"dailyAssignments",   icon:ICONS.daily,       label:"Daily Assignments" },
//   { id:"weeklyAssignments",  icon:ICONS.weekly,      label:"Weekly Assignments" },
//   { id:"sandbox",            icon:ICONS.sandbox,     label:"TERMINAL" },
...
// function AssignmentPlaceholder({ type, T }) {
//   const isDaily = type === "daily";
//   const color   = isDaily ? "#00b8a3" : "#8b5cf6";
//   const icon    = isDaily ? "📅" : "📆";
//   const label   = isDaily ? "Daily" : "Weekly";
//   return (
//     <div className="section-enter">
//       <div style={{marginBottom:"1.75rem"}}>
//         <h2 style={{fontWeight:800,fontSize:"1.75rem",color:T.text}}>{icon} {label} Assignments</h2>
//         <p style={{fontSize:"0.85rem",color:T.textFaint,marginTop:"0.35rem"}}>Track your {label.toLowerCase()} tasks and goals</p>
//       </div>
//       <div style={{
//         display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",
//         minHeight:"40vh",gap:"1.25rem",
//         background:T.bgCard,border:`1.5px dashed ${color}50`,borderRadius:"16px",padding:"3rem",
//       }}>
//         <div style={{
...
//           <div style={{fontWeight:800,fontSize:"1.25rem",color:T.text,marginBottom:"0.4rem"}}>
//             Coming Soon: {label} Assignments
//           </div>
//           <div style={{fontSize:"0.88rem",color:T.textFaint,maxWidth:380,lineHeight:1.6}}>
//             {isDaily
//               ? "Your personalised daily tasks will appear here. Check back soon!"
//               : "Weekly challenge sets and progress reports will be available here shortly."}
//           </div>
//         </div>
//         <span style={{
//           fontSize:"0.75rem",fontWeight:700,padding:"0.35rem 1.1rem",
//           borderRadius:"20px",background:`${color}14`,color,border:`1px solid ${color}35`,
...
//   const totalQ=questions.length;const myRank=leaderboard.findIndex(s=>s.id===currentUser?.uid)+1;
//   const completedExams=Object.values(dashboardSubmissions).filter(s=>s.status==="completed");
//   const avgScore=completedExams.length?Math.round(completedExams.reduce((acc,s)=>acc+(s.score||0),0)/completedExams.length):0;
//   const items=[{label:"Total Questions",val:totalQ,pct:Math.min(100,totalQ),color:"#ff6b35"},{label:"Exams Completed",val:completedExams.length,pct:Math.min(100,completedExams.length*10),color:"#00b8a3"},{label:"Avg Score",val:`${avgScore}%`,pct:avgScore,color:"#5a6cf5"},{label:"Global Rank",val:myRank>0?`#${myRank}`:"—",pct:myRank>0?Math.max(5,100-myRank*10):0,color:"#e05c7a"}];
//   return(
//     <div className="section-enter">
//       <div style={{marginBottom:"1.75rem"}}><h2 style={{fontWeight:800,fontSize:"1.75rem",color:T.text}}>📊 Performance Reports</h2></div>
...
//                     || s.questionCount
//                     || exams.find(e => e.id === s.examId)?.questionIds?.length
//                     || 1;
//       const correct  = s.score || s.correctCount || s.correctAnswers || 0;
//       const pct      = Math.min(correct / totalQ, 1);
//       return acc + Math.round(pct * 100);
//     }, 0);

//   const practiceAttempts = userProfile?.practiceAttempts
//                         || userProfile?.totalAttempts
//                         || 0;
...
//           <div>
//             <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"1.125rem"}}>
//               <div style={{fontSize:"1.1rem",fontWeight:700,color:T.text}}>Upcoming Exams</div>
...
//                 onMouseLeave={e=>{e.currentTarget.style.borderColor="#e5e7eb";e.currentTarget.style.color="#6b7280";}}>View all →</button>
//             </div>
//             {exams.length===0?<div style={{color:T.textFaint,fontSize:"0.9rem",padding:"1.75rem",textAlign:"center",border:`1px dashed ${T.border}`,borderRadius:"10px"}}>No exams assigned yet.</div>:(
//               <div style={{display:"flex",flexDirection:"column",gap:"0.625rem"}}>
//                 {exams.slice(0,3).map(exam=>{
//                   const ws=getWindowStatus(exam);const isLive=ws==="live";
//                   return(
//                     <div key={exam.id} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0.9rem 1.125rem",background:isLive?"rgba(0,184,163,0.06)":T.bgCard,border:`1px solid ${isLive?"rgba(0,184,163,0.3)":T.border}`,borderRadius:"10px",gap:"1.125rem"}}>
//                       <div>
//                         <div style={{fontSize:"0.9rem",fontWeight:600,color:T.text}}>{exam.title}</div>
//                         <div style={{...NUM_STYLE,fontSize:"0.75rem",color:T.textFaint,marginTop:"0.15rem"}}>{exam.scheduledStartTime?fmtDate(exam.scheduledStartTime):"Date TBD"} · {exam.durationMinutes??60} min</div>
//                       </div>
//                       {isLive?(
//                         dashboardSubmissions[exam.id]?.status==="completed"
//                           ? <span style={{fontSize:"0.72rem",fontWeight:700,padding:"0.25rem 0.7rem",borderRadius:"5px",background:"rgba(90,108,245,0.1)",color:"#5a6cf5",border:"1px solid rgba(90,108,245,0.3)"}}>✓ Submitted</span>
//                           : <button onClick={()=>handleStartExam(exam.id, dashboardSubmissions[exam.id]?.id||null)} style={{background:"#ff6b35",border:"none",borderRadius:"8px",padding:"0.5rem 1.125rem",color:"#fff",fontSize:"0.78rem",fontWeight:700,cursor:"pointer"}}>{dashboardSubmissions[exam.id]?"Resume →":"Start →"}</button>
//                       ):<span style={{...NUM_STYLE,fontSize:"0.72rem",fontWeight:600,padding:"0.25rem 0.7rem",borderRadius:"5px",background:"#f7f8fa",color:"#6b7280",border:"1px solid #e5e7eb"}}>{ws.charAt(0).toUpperCase()+ws.slice(1)}</span>}
//                     </div>
//                   );
//                 })}
//               </div>
...
//         {/* ── FIXED: Sidebar header with proper collapsed centering ── */}
//         <div style={{
//           padding: sidebarOpen ? "1.375rem 1.125rem 1.125rem" : "1.375rem 0 1.125rem",
...
//             display: "flex",
//             alignItems: "center",
//             gap: sidebarOpen ? "0.875rem" : 0,
//             width: "100%",
//             justifyContent: sidebarOpen ? "flex-start" : "center",
//           }}>
//             {/* Logo / Fallback icon — always centered when collapsed */}
//             {collegeLogo ? (
//               <img
//                 src={collegeLogo}
//                 alt="logo"
...
//                 <div style={{ fontWeight: 800, fontSize: "1.02rem", color: T.text, whiteSpace: "nowrap" }}>
//                   {collegeName || "Mind Code"}
//                 </div>
//                 <div style={{ fontSize: "0.6rem", color: "#ff6b35", letterSpacing: "0.12em", textTransform: "uppercase", fontWeight: 600 }}>
//                   Student Portal
//                 </div>
//               </div>
//             )}
...
//           {/* Toggle button — positioned right edge, vertically centered */}
//           <button
...
//                   justifyContent: sidebarOpen ? "flex-start" : "center",
//                   gap: sidebarOpen ? "0.875rem" : 0,
//                   padding: sidebarOpen ? "0.65rem 0.875rem" : "0.65rem 0",
//                   borderRadius:"9px",border:"none",
//                   borderLeft: activeNav===item.id
//                     ? `3px solid ${isSandbox?"#ffa116":"#ff6b35"}`
//                     : "3px solid transparent",
//                   background: activeNav===item.id
//                     ? (isSandbox?"rgba(255,161,22,0.1)":"#fff4f0")
//                     : "transparent",
//                   cursor:"pointer",
//                   color: activeNav===item.id
//                     ? (isSandbox?"#ffa116":"#ff6b35")
//                     : T.textMuted,
//                   fontSize:"0.9rem",
//                   fontWeight:activeNav===item.id?600:400,
//                   marginBottom:"0.15rem",textAlign:"left",transition:"all .15s",
//                 }}
...
//                 {sidebarOpen&&(
//                   <span style={{whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",display:"flex",alignItems:"center",gap:"0.4rem"}}>
//                     {item.label}
//                     {isSandbox&&(
//                       <span style={{fontSize:"0.55rem",fontWeight:700,padding:"0.1rem 0.4rem",borderRadius:"4px",background:"rgba(255,161,22,0.15)",color:"#ffa116",border:"1px solid rgba(255,161,22,0.3)",letterSpacing:"0.06em"}}>FREE</span>
//                     )}
//                   </span>
//                 )}
//               </button>
...
//                   style={{display:"flex",alignItems:"center",gap:"0.3rem",background:"transparent",border:`1px solid ${T.border}`,borderRadius:"7px",padding:"0.35rem 0.65rem",cursor:"pointer",fontSize:"0.65rem",color:"#6b7280",fontWeight:600,flexShrink:0,transition:"all .15s"}}
...
//             )}
//             {!sidebarOpen&&(
//               <button onClick={handleLogout} style={{display:"flex",alignItems:"center",justifyContent:"center",background:"transparent",border:"none",cursor:"pointer",color:T.textMuted,padding:"0.3rem",borderRadius:"5px",transition:"color .15s",marginLeft:"0.25rem"}} onMouseEnter={e=>e.currentTarget.style.color="#ef4444"} onMouseLeave={e=>e.currentTarget.style.color=T.textMuted}>{ICONS.logout}</button>
//             )}
//           </div>
//         </div>
//       </aside>
...
//         {/* ── Content area ── */}
//         <div style={{
//           padding: isFullscreen ? "0" : "2rem 2.25rem",
...
//               {activeNav==="dailyAssignments"  && <AssignmentPlaceholder type="daily"  T={T}/>}
//               {activeNav==="weeklyAssignments" && <AssignmentPlaceholder type="weekly" T={T}/>}
//               {activeNav==="sandbox" && (
...
//   // ── UPDATED: always show details modal for new attempts ───────────────────
//   const handleStartExam = (examId, existingSubmissionId = null) => {
//     if (!currentUser || !examId) return;
//     // Resume in-progress → go straight in (details already collected)
//     if (existingSubmissionId) {
//       history.push(`/exam/${examId}/${existingSubmissionId}`);
//       return;
//     }
//     // New attempt for any exam type (DAILY / WEEKLY / EXAM) → collect details first
//     setExamDetailsModal({ examId });
//   };
...
//           {/* ── UPDATED: Upcoming Exams with type badges ── */}
//           <div>
//             <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"1.125rem"}}>
//               <div style={{fontSize:"1.1rem",fontWeight:700,color:T.text}}>Upcoming Exams</div>
//               <button onClick={()=>setActiveNav("exams")} style={{background:"transparent",border:"1px solid #e5e7eb",borderRadius:"8px",padding:"0.375rem 0.9rem",color:"#6b7280",fontSize:"0.78rem",cursor:"pointer",fontWeight:600,transition:"all .15s"}}
...
//             </div>
//             {exams.length===0
//               ? <div style={{color:T.textFaint,fontSize:"0.9rem",padding:"1.75rem",textAlign:"center",border:`1px dashed ${T.border}`,borderRadius:"10px"}}>No exams assigned yet.</div>
//               : (
//               <div style={{display:"flex",flexDirection:"column",gap:"0.625rem"}}>
//                 {exams.slice(0,3).map(exam => {
...
//                       background: isLive ? "rgba(0,184,163,0.06)" : T.bgCard,
//                       border: `1px solid ${isLive ? "rgba(0,184,163,0.3)" : T.border}`,
...
//                               style={{background:"#ff6b35",border:"none",borderRadius:"8px",padding:"0.5rem 1.125rem",color:"#fff",fontSize:"0.78rem",fontWeight:700,cursor:"pointer",flexShrink:0}}>
//                               {sub ? "Resume →" : "Start →"}
//                             </button>
//                       ) : (
//                         <span style={{...NUM_STYLE,fontSize:"0.72rem",fontWeight:600,padding:"0.25rem 0.7rem",borderRadius:"5px",background:T.bgHover,color:T.textMuted,border:`1px solid ${T.border}`,flexShrink:0}}>
...
//             return(
//               <button key={item.id} onClick={()=>setActiveNav(item.id)} title={!sidebarOpen?item.label:undefined}
//                 style={{width:"100%",display:"flex",alignItems:"center",justifyContent:sidebarOpen?"flex-start":"center",gap:sidebarOpen?"0.875rem":0,padding:sidebarOpen?"0.65rem 0.875rem":"0.65rem 0",borderRadius:"9px",border:"none",borderLeft:activeNav===item.id?`3px solid ${isSandbox?"#ffa116":"#ff6b35"}`:"3px solid transparent",background:activeNav===item.id?(isSandbox?"rgba(255,161,22,0.1)":"#fff4f0"):"transparent",cursor:"pointer",color:activeNav===item.id?(isSandbox?"#ffa116":"#ff6b35"):T.textMuted,fontSize:"0.9rem",fontWeight:activeNav===item.id?600:400,marginBottom:"0.15rem",textAlign:"left",transition:"all .15s"}}
//                 onMouseEnter={e=>{if(activeNav!==item.id){e.currentTarget.style.background=dark?"#1c2128":"#f7f8fa";e.currentTarget.style.color=dark?"#e6edf3":"#1a1a1a";}}}
//                 onMouseLeave={e=>{if(activeNav!==item.id){e.currentTarget.style.background="transparent";e.currentTarget.style.color=T.textMuted;}}}>
//                 <span style={{display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>{item.icon}</span>
//                 {sidebarOpen&&(
//                   <span style={{whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",display:"flex",alignItems:"center",gap:"0.4rem"}}>
//                     {item.label}
//                     {isSandbox&&<span style={{fontSize:"0.55rem",fontWeight:700,padding:"0.1rem 0.4rem",borderRadius:"4px",background:"rgba(255,161,22,0.15)",color:"#ffa116",border:"1px solid rgba(255,161,22,0.3)",letterSpacing:"0.06em"}}>FREE</span>}
//                   </span>
//                 )}
//               </button>
//             );
...
//   const SW=sidebarOpen?256:76;
//   // const displayName=currentUser?.displayName||userProfile?.name||"Student";
//   const displayName=currentUser?.displayName||userProfile?.name||userProfile?.displayName||currentUser?.email?.split("@")[0]||"Student";
//   const firstName = (currentUser?.displayName || userProfile?.name || userProfile?.displayName || currentUser?.email?.split("@")[0] || "Student").split(" ")[0];
//   const firstName=displayName.split(" ")[0];
//   const initials=displayName.split(" ").map(n=>n[0]).slice(0,2).join("").toUpperCase();
...
//     .filter(s => s.status === "completed")
//     .reduce((acc, s) => {
//       const totalQ   = s.totalQuestions || s.questionCount || exams.find(e => e.id === s.examId)?.questionIds?.length || 1;
//       const correct  = s.score || s.correctCount || s.correctAnswers || 0;
//       const pct      = Math.min(correct / totalQ, 1);
//       return acc + Math.round(pct * 100);
//     }, 0);
